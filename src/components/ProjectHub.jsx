...
import { 
  ...
  Folder, ChevronDown, ListOrdered, Play, RefreshCw, AlertCircle, ShieldCheck, Github, Box, Monitor
} from 'lucide-react';
import { TRANSLATIONS } from '../data/translations';
import confetti from 'canvas-confetti';
import LiveSandbox from './LiveSandbox';

export default function ProjectHub({ projectData, onUpdateTopic, currentLang = 'en' }) {
  ...
  const [subTab, setSubTab] = useState('architecture'); // 'architecture' | 'sandbox' | 'tracker'
  ...
  // GitHub REST API REPOSITORY GENERATOR
...
models/`;
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-700 text-sm font-semibold mt-1">
              Interactive architecture diagrams, live code sandbox, real-time progress tracking, and 1-click GitHub repos for "{projectTitle}".
            </p>
          </div>
      ...
      {/* INTEGRATED SUB-NAV BAR: ARCHITECTURE | LIVE SANDBOX | PROGRESS TRACKER */}
      <div className="flex items-center bg-slate-100 p-1.5 rounded-2xl border border-slate-200 text-xs font-extrabold shadow-sm overflow-x-auto scrollbar-none">
        <button
          onClick={() => setSubTab('architecture')}
          className={`flex items-center space-x-2 px-5 py-2.5 rounded-xl transition cursor-pointer whitespace-nowrap ${
            subTab === 'architecture' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-700 hover:bg-slate-200'
          }`}
        >
          <Layers className="w-4 h-4" />
          <span>Architecture & Layer Code</span>
        </button>

        <button
          onClick={() => setSubTab('sandbox')}
          className={`flex items-center space-x-2 px-5 py-2.5 rounded-xl transition cursor-pointer whitespace-nowrap ${
            subTab === 'sandbox' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-700 hover:bg-slate-200'
          }`}
        >
          <Monitor className="w-4 h-4" />
          <span>Live Code Preview Sandbox</span>
        </button>

        <button
          onClick={() => setSubTab('tracker')}
          className={`flex items-center space-x-2 px-5 py-2.5 rounded-xl transition cursor-pointer whitespace-nowrap ${
            subTab === 'tracker' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-700 hover:bg-slate-200'
          }`}
        >
      ...
      {/* MODAL: GITHUB REST API REPOSITORY & BOILERPLATE GENERATOR */}
      {showRepoModal && (
      ...
      {/* SUB-TAB 1: LIVE CODE PREVIEW SANDBOX (INTEGRATED INSIDE PROJECT GENERATOR) */}
      {subTab === 'sandbox' && (
        <LiveSandbox projectData={projectData} onUpdateTopic={onUpdateTopic} currentLang={currentLang} />
      )}

      {/* SUB-TAB 2: REAL-TIME PROGRESS TRACKER */}
      {subTab === 'tracker' && (
        <div className="glass-panel p-6 sm:p-8 rounded-2xl space-y-5 bg-white border border-indigo-200 shadow-lg animate-fadeIn">
      ...
      {/* SUB-TAB 3: ARCHITECTURE & LAYER CODE */}
      {subTab === 'architecture' && (
        <div className="bg-slate-950 p-6 sm:p-10 rounded-3xl border border-slate-800 text-white space-y-8 shadow-2xl animate-fadeIn">
          
          <div className="space-y-3">
            <h3 className="text-2xl font-bold text-white tracking-tight">Project Generator</h3>
            <p className="text-sm text-slate-300 font-medium">Automatically generate architecture diagram and layer code for "{projectTitle}"</p>
