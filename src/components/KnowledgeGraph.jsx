...
import { Share2, Sparkles, Filter, BookOpen, Database, Code, Cpu } from 'lucide-react';
...
export default function KnowledgeGraph({ projectData, currentLang = 'en' }) {
  ...
  const nodes = [
    { id: 1, label: "EcoMeal Core Concept", category: "Core", x: 50, y: 50, color: "#6366f1", details: "Central AI engine combining plate waste computer vision with demand forecasting." },
    { id: 2, label: "YOLOv8 Segmentation", category: "AI Models", x: 25, y: 30, color: "#a855f7", details: "Instance segmentation model trained on 15,000 dining plate images." },
    { id: 3, label: "Prophet Attendance Model", category: "AI Models", x: 75, y: 30, color: "#a855f7", details: "Time-series forecasting based on academic calendar & historical gate biometric data." },
    { id: 4, label: "IEEE Food Waste Paper", category: "Papers", x: 20, y: 70, color: "#00f2fe", details: "Zhang et al. (2025) - Plate waste volume estimation accuracy at 94.2%." },
    { id: 5, label: "FoodLoss-Vision 15K", category: "Datasets", x: 80, y: 70, color: "#10b981", details: "Kaggle dataset with 15k annotated plate waste images." },
    { id: 6, label: "MongoDB Atlas Vault", category: "Tech Stack", x: 35, y: 80, color: "#3b82f6", details: "Live MongoDB Atlas document store with sub-20ms query SLA." },
    { id: 7, label: "WhatsApp Student Bot", category: "Tech Stack", x: 65, y: 80, color: "#f59e0b", details: "Interactive WhatsApp bot for meal opt-outs & hostel notifications." },
  ];
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="glass-panel p-4 rounded-2xl flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-xs text-slate-300 font-semibold">{t.clusterFilter}</span>
        ...
        <div className="text-xs text-slate-400">
          {t.showingNodes}: <span className="text-cyan-300 font-bold">{filteredNodes.length}</span>
        </div>
      </div>
        ...
        {activeNode && (
          <div className="absolute bottom-4 left-4 right-4 sm:left-auto sm:right-4 sm:w-80 glass-panel-glow p-4 rounded-xl border border-cyan-500/40 z-40 space-y-2 animate-scaleUp">
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => setActiveNode(null)} className="text-slate-400 hover:text-white text-sm cursor-pointer">✕</button>
            </div>
